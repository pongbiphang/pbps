...
pub enum DbError {
    ...
    /// SEAM 9: `pbps-db` opens the TCP stream itself and so holds the
    /// `io::Error`; tokio-postgres opens it inside `connect` and wraps the
    /// failure in its own error, so the seam's `Connect` is filled from that
    /// error's `source()`. A first draft of this spike folded the timeout
    /// into `BadConnectionString`, which is the one category the seam keeps
    /// apart on purpose: a firewall dropping packets and a malformed string
    /// need different fixes, and the CLI's diagnostics tell them apart.
    ...
    #[error("`{addr}` did not answer within {}s", CONNECT_TIMEOUT.as_secs())]
...
/// The address the seam names in `Connect` and `ConnectTimeout`. tiberius
/// exposes it as one string (`Config::get_addr`); tokio-postgres holds hosts
/// and ports as parallel lists, so the spike renders the first pair.
fn describe_addr(config: &tokio_postgres::Config) -> String {
...
/// SEAM 9, the other half: a refused or unreachable socket arrives as a
/// `tokio_postgres::Error` whose `source()` is the `io::Error`. The seam's
/// `Connect` wants that `io::Error` by value, and it cannot be moved out of the
/// driver's error, so it is rebuilt from its kind and text.
fn connect_error(addr: String, e: tokio_postgres::Error) -> DbError {
    use std::error::Error as _;
    match e.source().and_then(|s| s.downcast_ref::<std::io::Error>()) {
        Some(io) => DbError::Connect {
            addr,
            source: std::io::Error::new(io.kind(), io.to_string()),
        },
        None => DbError::Driver(e),
    }
}
...
/// Which `DbError` variant a failure landed in — the category is the finding,
/// the driver's text is not.
fn category(e: DbError) -> &'static str {
    match e {
        DbError::BadConnectionString(_) => "BadConnectionString",
        DbError::Connect { .. } => "Connect",
        DbError::ConnectTimeout { .. } => "ConnectTimeout",
        DbError::Driver(_) => "Driver",
        DbError::BadRow(_) => "BadRow",
    }
}

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // The three connection failures the seam distinguishes, before the run
    // proper. The third needs a socket that accepts and never answers — a
    // blackhole address is refused outright by a host with no route to it —
    // and waits the full CONNECT_TIMEOUT, so it only runs when asked for.
    let malformed = Conn::connect("host=127.0.0.1 port=notaport")
        .await
        .err()
        .map(category);
    let refused = Conn::connect("host=127.0.0.1 port=1 user=postgres connect_timeout=5")
        .await
        .err()
        .map(category);
    println!("malformed string: {malformed:?}");
    println!("refused socket:   {refused:?}");
    if let Ok(silent) = std::env::var("PG_SPIKE_SILENT") {
        let dropped = Conn::connect(&format!("{silent} user=postgres"))
            .await
            .err()
            .map(category);
        println!("never answered:   {dropped:?}");
    }

    let cs = std::env::var("PG_SPIKE_URL").unwrap_or_else(|_| {
        "host=127.0.0.1 port=55432 user=postgres password=probe dbname=postgres".to_owned()
    });
    let mut c = Conn::connect(&cs).await?;
    println!("connect: ok");

    c.execute("DROP SCHEMA IF EXISTS spike CASCADE; CREATE SCHEMA spike;")
        .await?;
    println!("execute (multi-statement batch): ok");

    let rows = c
        .query("SELECT 42 AS answer, NULL::text AS nothing")
        .await?;
    ...
    c.execute("CREATE TABLE spike.t (id int primary key, v text)")
        .await?;
    let n = c
        .execute_with(
            "INSERT INTO spike.t VALUES ($1, $2)",
            &[Param::I32(1), Param::Str("one")],
        )
        .await?;
    println!("execute_with: {n} row(s) affected");
    let rows = c
        .query_with("SELECT v FROM spike.t WHERE id = $1", &[Param::I32(1)])
        .await?;
    ...
    c.execute_with(
        "INSERT INTO spike.t VALUES ($1, $2)",
        &[Param::I32(2), Param::OptStr(None)],
    )
    .await?;
    let rows = c
        .query("SELECT v IS NULL AS is_null FROM spike.t WHERE id = 2")
        .await?;
    ...
    c.begin().await?;
    c.execute("INSERT INTO spike.t VALUES (10, 'first')")
        .await?;
    let failed = c
        .execute("INSERT INTO spike.t VALUES (1, 'duplicate key')")
        .await;
    ...
    c.rollback().await?;
    let rows = c
        .query("SELECT count(*) AS n FROM spike.t WHERE id = 10")
        .await?;
    ...
    if let Err(e) = &undefined {
        ...
        println!(
            "undefined table SQLSTATE: {code:?}, parses as u32: {:?}",
