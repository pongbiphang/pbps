...
/// column's sequence and a table's array type are two. Scratch makes them too,
/// and the target records them as the table's, so a view reading one has a
    ...
    for variable in SERVERS {
        ...
        let declared = || {
            ...
            Declared::default()
                ...
                .view("app.w", "SELECT NULL::_t AS y")
        };
        let assessment = analyze(
            &server,
            "created_sequence",
            Case {
                ...
                target: "
                    ...
                    CREATE VIEW app.w AS SELECT NULL::_t AS y;",
                base: declared(),
                desired: declared(),
            },
        )
...
/// name first, or an unmanaged type that took a table's array type name
/// first, shares the identity with scratch's copy without being it. An
...
async fn a_shared_identity_the_target_did_not_get_from_a_managed_object_is_unresolved() {
    ...
    for variable in SERVERS {
        ...
        let array = || {
            ...
            Declared::default()
                .table("app.t", table)
                .view("app.v", "SELECT NULL::_t AS y")
        };
        let indexed = || {
            let mut table = pbps_model::Table::default();
            table.columns.insert(
        ...
        for (tag, target, base, desired, extras) in [
            ...
            (
                "taken_array_type",
                "CREATE TYPE app._t AS (x integer);
                 CREATE TABLE app.t (x integer);
                 SET search_path = app;
                 CREATE VIEW app.v AS SELECT NULL::_t AS y;",
                array(),
                array(),
                &[][..],
            ),
