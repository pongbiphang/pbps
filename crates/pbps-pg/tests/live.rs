...
#[tokio::test]
#[ignore = "needs a live PostgreSQL; set PBPS_TEST_PG_DB (see scripts/live-tests-pg.sh)"]
async fn argument_free_cursor_operands_do_not_rebuild_for_view_arrivals() {
    let mut db = TestDb::create("cursor_view230").await;
    db.conn.execute("CREATE SCHEMA app").await.unwrap();
    let pg = Postgres::new();
    let bodies = [
        "DECLARE orders CURSOR FOR SELECT 7; BEGIN OPEN orders; RETURN 7; END",
        "DECLARE \"orders\" NO SCROLL CURSOR FOR SELECT 7; result int; BEGIN OPEN \"orders\"; FETCH \"orders\" INTO result; CLOSE \"orders\"; RETURN result; END",
        "DECLARE orders SCROLL CURSOR FOR VALUES (7), (7); result int; BEGIN OPEN orders; MOVE NEXT FROM orders; FETCH NEXT FROM orders INTO result; CLOSE orders; RETURN result; END",
        "BEGIN DECLARE orders CURSOR FOR SELECT 7; result int; BEGIN OPEN orders; FETCH FROM orders INTO result; CLOSE orders; RETURN result; END; END",
    ];
    let mut a = Schema::default();
    for (i, body) in bodies.iter().enumerate() {
        a.modules.insert(
...
#[tokio::test]
#[ignore = "needs a live PostgreSQL; set PBPS_TEST_PG_DB (see scripts/live-tests-pg.sh)"]
async fn foreign_key_targets_do_not_hide_real_default_calls() {
    let mut db = TestDb::create("fk_calls230").await;
    db.conn
        .execute(
            "CREATE SCHEMA app; CREATE SCHEMA shared; \
             CREATE FUNCTION shared.orders(integer) RETURNS int LANGUAGE sql AS 'SELECT 7'; CREATE TABLE shared.orders(id int PRIMARY KEY); INSERT INTO shared.orders VALUES (7),(42)",
        )
        .await
        .unwrap();
    let pg = Postgres::with_write_path_extras(vec!["shared".into()]);
    let bodies = [
        "DECLARE result int; BEGIN CREATE TABLE app.fk_call (id int DEFAULT orders(7) REFERENCES orders(id)); INSERT INTO app.fk_call DEFAULT VALUES; SELECT id INTO result FROM app.fk_call; DROP TABLE app.fk_call; RETURN result; END",
    ];
    let mut a = Schema::default();
    for (i, body) in bodies.iter().enumerate() {
        a.modules.insert(
...
#[tokio::test]
#[ignore = "needs a live PostgreSQL; set PBPS_TEST_PG_DB (see scripts/live-tests-pg.sh)"]
async fn ddl_expressions_keep_real_routine_calls() {
    let mut db = TestDb::create("ddl_calls230").await;
    db.conn
        .execute(
            "CREATE SCHEMA app; CREATE SCHEMA shared; \
             CREATE FUNCTION shared.numeric(integer,integer) RETURNS int LANGUAGE sql IMMUTABLE AS 'SELECT 7'",
        )
        .await
        .unwrap();
    let pg = Postgres::with_write_path_extras(vec!["shared".into()]);
    let bodies = [
        "DECLARE result int; BEGIN CREATE TEMP TABLE ddl_call (g numeric(10,2) DEFAULT \"numeric\"(1,2)); INSERT INTO ddl_call DEFAULT VALUES; SELECT g::int INTO result FROM ddl_call; DROP TABLE ddl_call; RETURN result; END",
    ...
    b.modules.insert(
        ...
        module(
            pbps_model::ModuleKind::Function,
            "(integer,integer) RETURNS int LANGUAGE sql IMMUTABLE AS 'SELECT 42'",
        ),
    );
    let arrival = plan(&a, &ids, &b, &ids);
    assert_eq!(arrival.changes.len(), bodies.len() + 1, "{arrival:#?}");
...
#[tokio::test]
#[ignore = "needs a live PostgreSQL; set PBPS_TEST_PG_DB (see scripts/live-tests-pg.sh)"]
async fn utility_target_column_lists_do_not_rebuild_for_routine_arrivals() {
    let mut db = TestDb::create("utility_names230").await;
    db.conn
    ...
    let statements = [
        "ANALYZE orders(id)",
        "ANALYSE VERBOSE orders(id)",
        "ANALYZE (VERBOSE false) source(id), orders(id)",
        "ANALYZE shared . \"orders\" /* columns */ (id)",
        "COPY orders(id) TO '/dev/null'",
        "COPY BINARY orders(id) TO '/dev/null'",
    ];
    let mut a = Schema::default();
...
#[tokio::test]
#[ignore = "needs a live PostgreSQL; set PBPS_TEST_PG_DB (see scripts/live-tests-pg.sh)"]
async fn foreign_key_targets_do_not_rebuild_for_routine_arrivals() {
    let mut db = TestDb::create("fk_targets230").await;
    db.conn
        .execute("CREATE SCHEMA app; CREATE SCHEMA shared; CREATE TABLE shared.orders(id int PRIMARY KEY); INSERT INTO shared.orders VALUES (7); CREATE TABLE shared.source(id int)")
        .await
        .unwrap();
    let pg = Postgres::with_write_path_extras(vec!["shared".into()]);
    let statements = [
        "CREATE TABLE child_inline (id int REFERENCES orders(id))",
        "CREATE TABLE child_table (id int, FOREIGN KEY(id) REFERENCES orders(id))",
        "CREATE TABLE child_add (id int); ALTER TABLE child_add ADD FOREIGN KEY(id) REFERENCES orders(id)",
        "CREATE TABLE child_qualified (id int REFERENCES shared.\"orders\"(id))",
    ];
    let mut a = Schema::default();
    for (i, statement) in statements.iter().enumerate() {
        a.modules.insert(
...
#[tokio::test]
#[ignore = "needs a live PostgreSQL; set PBPS_TEST_PG_DB (see scripts/live-tests-pg.sh)"]
async fn ddl_column_types_do_not_rebuild_for_routine_arrivals() {
    let mut db = TestDb::create("ddl_types230").await;
    db.conn
        .execute("CREATE SCHEMA app; CREATE SCHEMA shared; CREATE TABLE shared.orders(id int); INSERT INTO shared.orders VALUES (7); CREATE TABLE shared.source(id int)")
        .await
        .unwrap();
    let pg = Postgres::with_write_path_extras(vec!["shared".into()]);
    let statements = [
        "CREATE TEMP TABLE ddl_basic (g numeric(10,2))",
        "CREATE TEMPORARY TABLE IF NOT EXISTS ddl_quoted (g \"numeric\"(10,2))",
        "CREATE TEMP TABLE ddl_add (id int); ALTER TABLE ddl_add ADD COLUMN g numeric(10,2)",
        "CREATE TEMP TABLE ddl_type (g int); ALTER TABLE ONLY ddl_type ALTER COLUMN g TYPE numeric(10,2)",
...
#[tokio::test]
#[ignore = "needs a live PostgreSQL; set PBPS_TEST_PG_DB (see scripts/live-tests-pg.sh)"]
async fn index_access_methods_do_not_rebuild_for_routine_arrivals() {
    let mut db = TestDb::create("index_methods230").await;
    db.conn
        .execute("CREATE SCHEMA app; CREATE SCHEMA shared; CREATE TABLE shared.orders(id int); INSERT INTO shared.orders VALUES (7); CREATE TABLE shared.source(id int)")
        .await
        .unwrap();
    let pg = Postgres::with_write_path_extras(vec!["shared".into()]);
    let statements = [
        "CREATE INDEX ix_basic ON orders USING btree (id)",
        "CREATE UNIQUE INDEX ix_unique ON ONLY orders USING \"btree\" (id)",
        "CREATE INDEX ix_expression ON shared.orders USING btree ((abs(id)))",
    ];
    let mut a = Schema::default();
    for (i, statement) in statements.iter().enumerate() {
        a.modules.insert(
    ...
    let mut b = a.clone();
    b.modules.insert(
        "app.btree(integer)".parse().unwrap(),
        module(
            pbps_model::ModuleKind::Function,
            "(integer) RETURNS int LANGUAGE sql AS 'SELECT 42'",
        ),
