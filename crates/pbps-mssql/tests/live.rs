...
async fn the_pull_keeps_a_project_table_named_like_the_ledger_with_a_trailing_space() {
    ...
    for theirs in ["__pbps_state ", "__pbps_lock "] {
        assert!(
...
async fn the_pull_hides_this_tools_two_tables_and_not_a_projects_own() {
    ...
    pbps_mssql::state::ensure_tables(&mut db.conn)
        .await
        .expect("create the ledger");
    // The lock table goes, so its name in another case can be a project's own
    // table below without colliding with it on a case-insensitive database.
    db.conn
        .execute("DROP TABLE dbo.__pbps_lock;")
        .await
        .expect("drop the lock table");
