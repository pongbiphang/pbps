...
/// A current command is a new human decision, not retained provenance. Even
/// when a loaded annotation can rename the same source elsewhere, an occupied
/// command target must be refused without writing that other identity result.
#[test]
fn an_occupied_rename_command_cannot_borrow_a_loaded_annotations_match() {
    let d = Demo::new("occupied-command-beside-annotation");
    d.table(
        "table: dbo.t\ncolumns:\n  id: {type: bigint, nullable: false}\n  old: {type: int}\n  occupied: {type: int}\n",
    );
    assert_eq!(code(&d.run(&["plan"])), 0);
...
fn a_referenced_key_replacement_applies_and_the_next_plan_is_empty() {
    ...
    for kind in ["index", "unique", "primary"] {
        for staged in [false, true] {
            ...
            on_server(
                connection,
                "INSERT INTO dbo.t (id) VALUES (1); INSERT INTO dbo.child (id) VALUES (1);",
            );
            ...
            assert!(
                try_on_server(connection, "INSERT INTO dbo.child (id) VALUES (2)").is_err(),
...
fn external_referenced_key_dependencies_are_refused_before_planning_and_writing() {
    ...
    for kind in ["index", "unique", "primary"] {
        ...
        let external =
            "CREATE TABLE outside.child (id int CONSTRAINT external_fk REFERENCES dbo.t(id))";
        ...
        on_server(
            connection,
            "CREATE TABLE outside.unrelated (code int CONSTRAINT unrelated_fk REFERENCES dbo.t(code))",
        );
