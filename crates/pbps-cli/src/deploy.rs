...
fn refuse_unplanned_movement(
    ...
    for (name, was_rows) in touched_tables {
        ...
        if let (Some(was), Some(now)) = (before.tables.get(name), after.tables.get(now_name)) {
            ...
            let mut moved_parts = constraints.get(now_name).unwrap_or(&no_names).clone();
            if settled != Settled::Closing
                && let Some(declared) = created.get(now_name)
            {
                // PostgreSQL emits a CREATE TABLE's parts in later statements.
                // Once the table exists in a checkpoint, those additions still
                // belong to the CREATE payload (decision 458). Only an absent
                // -> present transition is exempt: a part already recorded in
                // a checkpoint must still compare by its read-back definition.
                use pbps_model::Part;
                macro_rules! pending {
                    ($field:ident, $kind:ident) => {
                        for n in declared.$field.keys() {
                            if !was.$field.contains_key(n) && now.$field.contains_key(n) {
                                moved_parts.insert((Part::$kind, n.as_str()));
                            }
                        }
                    };
                }
...
pub fn cmd_plan_db(
    ...
    if let Some(path) = sql_out {
        let script = format!(
            ...
            pbps_dialect::render_script(&statements, dialect.as_ref())
        );
        std::fs::write(path, script)
            .with_context(|| format!("cannot write `{}`", path.display()))?;
        if !json {
    ...
    for (i, stmt) in statements.iter().enumerate().skip(start) {
        ...
        let executed = if stmt.row_write.is_some() {
            ...
            let result = async {
                ...
                // A deferred trigger would otherwise run at COMMIT, after the
                // check below, and a function replaced while an earlier one
                // held the commit would run unchecked. The step commits right
                // after, so settling here refuses nothing the commit would
                // have allowed (473).
                crate::engine::settle_data_writes(conn).await?;
...
mod tests {
    ...
    #[test]
    fn a_created_tables_pending_parts_are_added_but_recorded_parts_still_compare() {
        use pbps_model::{
            CheckConstraint, ForeignKey, Index, IndexColumn, Table, UniqueConstraint,
        };
        let name = TableName::new("app", "t");
        ...
        declared.checks.insert(
            "ck".into(),
            CheckConstraint {
                expression: "id > 0".into(),
            },
        );
        declared.indexes.insert(
            "ix".into(),
            Index {
                columns: vec![IndexColumn {
                    name: "id".into(),
        ...
        let schema = |table| Schema {
            tables: [(name.clone(), table)].into(),
            ..Default::default()
        };
        let before = schema(bare);
        // The stored CHECK text is deliberately not the declaration's text.
        let mut recorded = declared;
        recorded.checks.get_mut("ck").unwrap().expression = "([id]>(0))".into();
        let after = schema(recorded.clone());
        for settled in [Settled::SoFar, Settled::Whole] {
            refuse_unplanned_movement(
                &pbps_mssql::Mssql,
                &changes,
            ...
            .expect("the CREATE payload's parts are emitted after the table");
        }
        refuse_unplanned_movement(
            &pbps_mssql::Mssql,
            &changes,
        ...
        .expect("unchanged recorded parts still close");
        refuse_unplanned_movement(
            &pbps_mssql::Mssql,
            &changes,
            &before,
        ...
        .expect_err("no statement runs between the final checkpoint and the closing read");
        for part in [
            "unique",
            "foreign key",
            "check",
            ...
            match part {
                "unique" => changed.unique.get_mut("uq").unwrap().columns = vec!["other".into()],
                "foreign key" => {
                    changed
                        .foreign_keys
                        .get_mut("fk")
            ...
            for settled in [Settled::SoFar, Settled::Whole, Settled::Closing] {
                ...
                .expect_err("a recorded part or a name outside the CREATE payload changed");
            }
        }
    }
