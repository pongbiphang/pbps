...
code lives in. The "Limits" section and the successor compose ADR distinguish retained
measurements from the remaining production qualification.
...
### 5. Git through the command line, with an isolated candidate and output branch

[ADR-0017](ADR-0017-isolated-compose.md) supersedes the same-checkout compose
protocol formerly specified here (#744, DECISIONS 527). The ordinary intent CLI
runs against a captured private snapshot. The page reviews one immutable tree;
confirmation creates that exact commit on a fresh output branch and pushes it
to one reviewed destination. Compose does not edit the source working files,
install the user's index, move its HEAD or current branch, or undo those
resources during recovery. The source keeps its edits and old ids; continued
editing uses an explicitly selected separate checkout of the result branch.

The Git subprocess boundary remains: user identity and signing policy, bounded
helpers and transport, disabled hooks/filters, exact output and no unrelated
history. Publication uncertainty and private-record cleanup still need durable
reconciliation. ADR-0017 states these obligations, the ownership tradeoff,
retention and continuation behavior, and the #745–#748 delivery split. It also
supersedes PR #738's proposed DECISIONS 524 live-placement preview; that proposal
is not an implemented baseline guarantee.

The former protocol remains available in
[the pre-decision revision](https://github.com/pongbiphang/pbps/blob/9c487db0/docs/ADR-0015-local-ui-implementation.md#5-git-through-the-git-command-not-a-library--and-the-commit-through-plumbing-which-runs-no-hook).
Its historical experiments and decisions remain evidence, not instructions to
build source-placement recovery before removing it. Legacy experimental records
must be recognized and refused safely as ADR-0017 specifies.
...
- **Compose publication and recovery.** ADR-0017 replaces the former live
  placement, HEAD and index protocol. Its retained experiment measures the
  isolated mechanism on real Git and CLI; #745–#748 must qualify the complete
  captured input manifest, browser boundary, ref outcome reconciliation and
  private-resource durability before enabling compose. The old #152/#385/#386
  recovery scenarios are historical obligations of the superseded design;
  legacy records are refused for explicit recovery, not discarded. Windows
  compose remains disabled until the selected protocol is qualified there.
