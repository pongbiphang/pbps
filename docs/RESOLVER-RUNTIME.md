...
| Stability | The scope is sealed to the target and scratch connections, so a reopened session cannot inherit it, and requalified on every check, so an extension, setting, collation or authorization change between checks invalidates the run |
