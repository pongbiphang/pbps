...
- **Writes use a path built per statement: the object's own schema first among
  the schemas the path names, then the project's configured extras, then
  `pg_temp` — and the path each module was created under is recorded with it.**
  `pg_catalog` is
  ...
  moves it (DECISIONS 277). In contrast, `pg_temp` must be named to keep a
  session's temporary relations and types behind the project schemas. The
  emitter places that exact alias once at the end, also when configured
  earlier among the extras (DECISIONS 464). The project extras are ordered, so
  their order is part of what a declaration *means*, and **measured**, a view keeps the binding it was
