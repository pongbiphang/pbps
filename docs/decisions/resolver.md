...
type or a call (its row type can take `t(x)`), a later routine for a routine
...
is looked up as a routine too and a bound routine name as a type, since
`t(x)` is an exact call, else a cast to `t`, else the best-matching call —
