...
the filesystem type and the mount and link walk of `objects`, `refs` and
`logs`, on the destination's Git common directory before the candidate
seals. `transport::run_push` runs them again just before pushing, since the
